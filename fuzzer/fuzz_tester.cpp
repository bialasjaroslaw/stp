...
void count_needle(const uint8_t* Data, size_t Size) {
  ...
  for (auto text_size = 1; text_size < Size - 1; ++text_size) {
    ...
    const std::basic_string_view<uint8_t> needle(Data + text_size,
                                                 Size - text_size);
...
void replace_needle(const uint8_t* Data, size_t Size) {
  ...
  for (int text_size = 1; text_size < Size - 1; ++text_size) {
    for (int needle_size = 0; needle_size < Size - text_size - 1;
         ++needle_size) {
      const std::basic_string_view<uint8_t> needle(Data + text_size,
                                                   needle_size);
      const std::basic_string_view<uint8_t> replace(
          Data + text_size + needle_size, Size - text_size - needle_size);
      std::basic_string<uint8_t> data(Data, text_size);
      auto reset = [&data, Data, text_size]() {
        data.resize(text_size);
...
void split_needle(const uint8_t* Data, size_t Size) {
  ...
  for (int text_size = 1; text_size < Size - 1; ++text_size) {
    ...
    for (int start_idx = -text_size; start_idx <= text_size; ++start_idx) {
      for (int end_idx = -text_size; end_idx <= text_size; ++end_idx) {
        for (int step = -step_limit; step <= step_limit; ++step) {
          Text::split(data, needle, Text::SplitBehavior::KeepEmpty, start_idx,
                      end_idx, step);
          Text::split_any(data, needle, Text::SplitBehavior::KeepEmpty,
                          start_idx, end_idx, step);
          Text::split_if(data, pred, Text::SplitBehavior::KeepEmpty, start_idx,
                         end_idx, step);
          Text::split(data, needle, Text::SplitBehavior::DropEmpty, start_idx,
                      end_idx, step);
          Text::split_any(data, needle, Text::SplitBehavior::DropEmpty,
                          start_idx, end_idx, step);
          Text::split_if(data, pred, Text::SplitBehavior::DropEmpty, start_idx,
                         end_idx, step);
        }
      }
    }
